...
export default function AdminProductsPage() {
  ...
  const columns = [
    ...
    { title: "Slug", dataIndex: "slug", key: "slug" },
    {
      ...
      dataIndex: "isActive",
      key: "isActive",
      render: (v: boolean) => <Tag color={v ? "green" : "red"}>{v ? "Hoạt động" : "Ngừng"}</Tag>,
    },
    {
      title: "Ngày tạo",
      dataIndex: "createdAt",
    ...
    },
    {
      ...
      render: (_: unknown, record: Product) => (
        <Space>
          <Button size="small" icon={<EditOutlined />} onClick={() => handleOpenProductModal(record)}>
            Sửa
          </Button>
          <Button size="small" icon={<PlusOutlined />} onClick={() => handleOpenPlanModal(record.id)}>
            Thêm gói
          </Button>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={() => handleOpenProductModal()}>
          Thêm sản phẩm
        </Button>
      </div>
      ...
      >
        ...
        >
          <Form.Item name="name" label="Tên gói" rules={[{ required: true }]}>
            <Input placeholder="VD: Pro - 1 năm" />
          </Form.Item>
          ...
          <Form.Item name="features" label='Tính năng (JSON array)' rules={[{ required: true }]} initialValue="[]">
            <TextArea rows={2} placeholder='["feature1","feature2"]' />
          </Form.Item>
        </Form>
      </Modal>
    </div>
...
function PlansTable({ productId }: { productId: string }) {
  ...
  const planColumns = [
    { title: "Tên gói", dataIndex: "name", key: "name" },
    {
      ...
      key: "durationDays",
      render: (v: number) => v === 0 ? "Vĩnh viễn" : `${v} ngày`,
    },
    ...
    },
    {
      ...
      dataIndex: "isActive",
      key: "isActive",
      render: (v: boolean) => <Tag color={v ? "green" : "red"}>{v ? "Bán" : "Ngừng"}</Tag>,
    },
    {
      title: "",
      key: "actions",
